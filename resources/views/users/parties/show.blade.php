...
@section('content')
    ...
    <div>
        ...
        <div class="clearfix">
            <div class="col col_6 col_md col_6_md icon_footer"><br>
                ...
                <i class="fab fa-internet-explorer"></i><span><strong>Կայք՝</strong><a href="{{ $party->site }}"> {{ $party->site }}</a></span><br>
                <i class="fab fa-facebook-f"></i><span><strong>ֆեյսբուք՝</strong><a href="{{ $party->fb }}"> {{ $party->fb }}</a></span><br>
            </div>
            <div class="col col_6 col_md col_6_md icon_footer"><br>
                <p class="p_post">Աշխատանքային օրեր, ժամեր</p>
                <i class="fas circle">1</i><span><strong>Երկուշաբթի՝</strong> {{ $party->monday }}</span><br>
                <i class="fas circle">2</i><span><strong>Երեքշաբթի՝</strong> {{ $party->tuesday }}</span><br>
                <i class="fas circle">3</i><span><strong>Չորեքշաբթի՝</strong> {{ $party->wednesday }}</span><br>
                <i class="fas circle">4</i><span><strong>Հինգշաբթի՝</strong> {{ $party->thursday }}</span><br>
                <i class="fas circle">5</i><span><strong>Ուրբաթ՝</strong> {{ $party->friday }}</span><br>
                <i class="fas circle">6</i><span><strong>Շաբաթ՝</strong> {{ $party->saturday }}</span><br>
                <i class="fas circle">7</i><span><strong>Կիրակի՝</strong> {{ $party->sunday }}</span><br>
            </div>
        </div>
    ...
    <div class="col-md-9 offset-md-3 margin_15_0">
        <a href="{{ route('users.parties.edit', $party->id) }}" class="btn btn-info button1 button1_text bg_edit">Edit</a>
        <a href="{{ route('users.parties.destroy', $party->id) }}" class="btn btn-delete btn-outline-danger button1 button1_text bg_delete">Delete</a>
    ...
    <form action="{{ route('tablichka.email') }}" method="post">
        @csrf
        <input type="hidden" value="{{ asset('storage/uploads/image/qr/'. $party->qr_cod) }}" name="image_qr">
        <input type="hidden" value="{{ $party->title }}" name="title">
    ...
    <div class="clearfix cucanak">
        <div class="col col_5 col_md col_5_md col_sm col_4_sm">
            <img src="{{ asset('storage/uploads/image/qr/'. $party->qr_cod) }}" alt="image format png" />
        </div>
